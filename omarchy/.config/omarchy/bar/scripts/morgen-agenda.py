#!/usr/bin/env python3
"""omarchy-shell bar tooltip: agenda from morgen-calendar CLI (Personal/Family/Work).

Emits Waybar-style JSON ({"text", "tooltip"}) on stdout, which the omarchy
bar's `type: "command"` module understands. The tooltip is rendered as plain
text by Quickshell, so titles are passed through unescaped.
"""
import json
import os
import re
...
def pad_block(lines: list[str]) -> list[str]:
    """Right-pad every line to the same width.

    The bar renders tooltips with horizontalAlignment: AlignHCenter, which
    centres each line individually and so shreds the time column. Equal-width
    lines in a monospace font all start at the same x, which is what we want.
    """
