...
Item {
  ...
  function lockSystem(reason) {
    ...
    // sspaeti: route through the wrapper that skips locking 1Password
    // (stock omarchy-system-lock runs `1password --lock`; no fingerprint here,
    // so every idle lock would force a full master-password unlock).
    runProcess(lockProcess, "lock", root.home + "/.config/hypr/sspaeti/omarchy-system-lock-wrapper.sh")
  }
