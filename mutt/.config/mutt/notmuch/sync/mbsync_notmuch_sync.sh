...
# mbsync + Notmuch sync script for neomutt (Linux/Arch)
# Syncs email with mbsync and applies notmuch screening tags

# Log file path
LOG_FILE=~/.config/mutt/logs/mbsync_notmuch_sync_status.log
...
# Log start time
echo "mbsync + Notmuch sync started at $(date)" >> "$LOG_FILE"

# Check if connected to Wi-Fi
if check_wifi; then
    # Run the first sync with mbsync
    echo "Running mbsync..." >> "$LOG_FILE"
    mbsync sspaeti.com
    ...
    echo "Running second mbsync..." >> "$LOG_FILE"
    mbsync sspaeti.com

    echo "mbsync + Notmuch sync completed successfully at $(date)" >> "$LOG_FILE"
else
